...
struct CSRListObject
{
	using data_type = typename List::data_type;
	using size_type = typename List::size_type;

	explicit CSRListObject(const List& list, size_type index) : _plist(&list), _index(index) {}
	...
	std::vector<data_type> data() const {
		...
		const auto it1 = _plist->data().begin() + _plist->offset().at(_index + 1);
