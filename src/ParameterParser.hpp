...
class ParameterParser
{
	...
	void resolve_cmd_line_args() {
		...
		_description.add_options()
			...
			("input,i", po::value<std::string>(), "the input mesh file (gmsh only)")
			("input_fmt", po::value<std::string>(&input_fmt)->default_value("msh"), "format of the input file (gmsh only)")
			("num,n", po::value<int>(), "parititon the mesh into #n parts")
			("periodic,p", po::value<std::string>(), "the file on nodal mapping about periodic BC")
			("output,o", po::value<std::string>(), "the output mesh file")
			("output_fmt", po::value<std::string>(&output_fmt)->default_value("h5"), "format of the output mesh file")
			;

		po::positional_options_description p_desc;
		p_desc.add("input", -1);

		po::store(po::command_line_parser(_argc, _argv).options(_description).positional(p_desc).run(),
				_arg_map);
...
inline std::ostream& operator<<(std::ostream& os, const ParameterParser& p) {
	std::vector<std::string> keys = {"help", "input", "input_fmt", "num", "periodic", "output", "output_fmt"};
